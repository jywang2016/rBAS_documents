...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rBAS}\OperatorTok{::}\KeywordTok{run_BAS_App}\NormalTok{(}\DataTypeTok{theme =} \StringTok{'united'}\NormalTok{)}
\CommentTok{#从rBAS包中调用run_BAS_App函数，theme参数是用户界面的主题}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(rBAS) }\CommentTok{#加载rBAS包，此后调用函数就不需要 rBAS::}
\KeywordTok{run_BAS_App}\NormalTok{(}\DataTypeTok{theme =} \StringTok{'united'}\NormalTok{) }\CommentTok{#直接调用函数}
\end{Highlighting}
\end{Shaded}

可以看到\texttt{rBAS}的用户界面，里面有关于\texttt{rBAS}的作者信息。\texttt{Rstudio-connect}支持网页上嵌入\texttt{shiny}界面的骚操作，有待之后探究，所以用截图\ref{fig:authors}来展示该作者信息界面。

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/Authors} 

}

\caption{用户界面作者信息}\label{fig:authors}
\end{figure}
